...
function main_track_states.shift2.transition(this,context,input)
    ...
    if (input == INPUT_BAYONET_MUZZLE) then
        ...
        local animationName = "auto_melee_" .. tostring(counter + 1)
...
function main_track_states.idle.transition(this, context, input)
    if (input == INPUT_FIRE_SELECT) then
        context:runAnimation("semi_to_auto", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.shift2
    end
    if (context:getFireMode() == BURST) then

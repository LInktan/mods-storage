...
local function runshiftPutAwayAnimation(context)
    local put_away_time = context:getPutAwayTime()
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    -- 播放 put_away 动画，并且将其剩余时长设为 context 传入的 put_away_time
    context:runAnimation("put_away", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
...
local function runshift2PutAwayAnimation(context)
    local put_away_time = context:getPutAwayTime()
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    -- 播放 put_away 动画，并且将其剩余时长设为 context 传入的 put_away_time
    context:runAnimation("put_away", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
...
local function runShiftReloadAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    if (isNoAmmo(context)) then
        context:runAnimation("reload_empty_scattering", track, false, PLAY_ONCE_STOP, 0.2)
    else
        context:runAnimation("reload_tactical_scattering", track, false, PLAY_ONCE_STOP, 0.2)
    end
end

local function runShift2ReloadAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    if (isNoAmmo(context)) then
        context:runAnimation("reload_empty_irradiating", track, false, PLAY_ONCE_STOP, 0.2)
    else
        context:runAnimation("reload_tactical_irradiating", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
local function runShiftInspectAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    if (isNoAmmo(context)) then
        context:runAnimation("inspect_empty_scattering", track, false, PLAY_ONCE_STOP, 0.2)
    else
        context:runAnimation("inspect_scattering", track, false, PLAY_ONCE_STOP, 0.2)
    end
end

local function runShift2InspectAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    if (isNoAmmo(context)) then
        context:runAnimation("inspect_empty_irradiating", track, false, PLAY_ONCE_STOP, 0.2)
    else
        context:runAnimation("inspect_irradiating", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
function main_track_states.start.transition(this, context, input)
    ...
    if (input == INPUT_DRAW) then
        if (context:getFireMode() == BURST) then
            context:runAnimation("draw_scattering", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
            return this.main_track_states.shift
        end
    end
    if (input == INPUT_DRAW) then
        if (context:getFireMode() == AUTO) then
            context:runAnimation("draw_irradiating", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
            return this.main_track_states.shift2
        end
    end
end

function main_track_states.shift.entry(this, context)
    context:runAnimation("scattering_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
end

function main_track_states.shift2.entry(this, context)
    context:runAnimation("irradiating_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
end
...
function main_track_states.shift.transition(this,context,input)
    if (input == INPUT_RELOAD) then
        runShiftReloadAnimation(context)
        return this.main_track_states.shift
    end
    if (input == INPUT_PUT_AWAY) then
        runshiftPutAwayAnimation(context)
        return this.main_track_states.final
    end
    if (input == INPUT_BOLT) then
        context:runAnimation("bolt_scattering", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.shift
    end
    if (input == INPUT_FIRE_SELECT) then
        context:runAnimation("irradiating", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.shift2
    end
    if (context:getFireMode() == AUTO) then
        context:runAnimation("irradiating_mode", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return main_track_states.shift2
    end
    if (input == INPUT_INSPECT) then
        runShiftInspectAnimation(context)
        return this.main_track_states.shift
    end
end

function main_track_states.shift2.transition(this,context,input)
    if (input == INPUT_RELOAD) then
        runShift2ReloadAnimation(context)
        return this.main_track_states.shift2
    end
    if (input == INPUT_PUT_AWAY) then
        runshift2PutAwayAnimation(context)
        return this.main_track_states.final
    end
    if (input == INPUT_BOLT) then
        context:runAnimation("bolt_irradiating", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.shift2
    end
    if (input == INPUT_FIRE_SELECT) then
        context:runAnimation("focusing", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.idle
    end
    if (context:getFireMode() == SEMI) then
        context:runAnimation("focusing_mode", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return main_track_states.idle
    end
    if (input == INPUT_INSPECT) then
        runShift2InspectAnimation(context)
        return this.main_track_states.shift2
    end
end
...
function main_track_states.idle.transition(this, context, input)
    if (input == INPUT_FIRE_SELECT) then
        context:runAnimation("scattering", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.shift
    end
    if (context:getFireMode() == BURST) then
        context:runAnimation("scattering_mode", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return main_track_states.shift
    end
    if (input == INPUT_PUT_AWAY) then
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == AUTO) then
            context:runAnimation("shoot_irradiating", track, true, PLAY_ONCE_STOP, 0) -- 这里是混合动画，一般是可叠加的 gun kick
        end
    end
    return nil
end
